...
class Cpphk extends CI_Controller {
    private $sTable = 'master_pph21_komponen';
	public function __construct(){
        parent::__construct();
        if ($this->session->userdata('status') != "login" && $this->session->userdata('level') != "SYS"){
            redirect(base_url("clogin"));
        }
        $this->load->model('mpphk','pphk');
	}

	public function index(){
        $this->header();
        $this->load->view('pphk_view', NULL);
	}
            ...
            foreach ($aColumns as $col) {
                ...
                $output = array(
                    ...
                    foreach ($rResult->result_array() as $aRow) {
                        $row = array();
                            $btn = '<a class="btn btn-sm text-dark" title="Delete" href="cpphk/delete/'.$aRow['kode_gaji'].'" onclick="return confirm(\'Anda yakin?\')"><i class="fa fa-trash-o"></i></a>';
                            for ( $i=0 ; $i<$iColumnCount ; $i++ ) {
                                $row[] = $aRow[ $aColumns[$i] ];
                            }
	...
	public function delete($kode_gaji){
        $this->pphk->delete_by_id($kode_gaji);
    ...
    function tambah(){
        ...
        $this->header();
        $this->load->view('pphk_add', $data);
    }

    function create(){
        $last = date("Y-m-d h:i:s");
        $data = array(
            'kode_gaji' => toAscii($this->input->post('kode_gaji')),
            'lastupdate' => $last,
            'user_id' => $this->session->userdata('nama')
        );
        $this->pphk->add_record($data);
        redirect('cpphk');
    }
     function header(){
