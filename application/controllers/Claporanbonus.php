...
class Claporanbonus extends CI_Controller {
    ...
    public function bonus($nik,$periode) {
        ...
        $pdf->SetTitle('Laporan Bonus');
        $pdf->SetSubject('Laporan Bonus');
        ...
        $qlaporan = $this->db->query("SELECT * FROM trans_periode_pph21_bonus WHERE periode='$periode' and nik='$nik'")->row();
        ...
        $html = '
            <table width="100%" border="0" cellpadding="1" cellspacing="1" style="font-size:8px;">
                <tr>
                    <td colspan="2" align="left">Bonus Slip : '.$per.'</td>
            ...
            ';
                ...
                $left=$left.'
                    <tr>
                        ...
                        <td align="right">'.number_format($qlaporan->bonus_tahunan,0,',','.').'</td>
                    </tr>
                ';

                if ($qlaporan->metode_pph21 == 'NET'){
                    $rqjabatan = round($qlaporan->pph21_bonus);
                    $left = $left.'
                        <tr>
                            <td align="left">Tunjangan PPH21 Bonus</td>
                            <td align="right">'.number_format($rqjabatan,0,',','.').'</td>
                        </tr>
                    ';
                    $tot    = $qlaporan->bonus_tahunan + $qlaporan->pph21_bonus;
        ...
        $right='
            ...
            ';
                ...
                if ($qlaporan->metode_pph21 == 'GROSS'){
                    ...
                    $right = $right.'
                        <tr>
                            <td align="left">Tax PPH21 Bonus</td>
                            <td align="right">'.number_format($rqjabatan,0,',','.').'</td>
                        </tr>
                        
                    ';
                    $qpppm = $qlaporan->pph21_bonus;
