...
class Cbank extends CI_Controller {
	...
	public function index(){
        $this->header();
        $this->load->view('bank_view', NULL);
	}

    function list_bank() {
        $aColumns = array('bank','lastupdate','user_id','id_bank');
        $sIndexColumn = 'bank';
        $input =& $_POST;
            ...
            foreach ($aColumns as $col) {
                ...
                $output = array(
                    ...
                    foreach ($rResult->result_array() as $aRow) {
                        $row = array();
                            $btn = '<a class="text-dark mr-1" href="cbank/delete/'.$aRow['id_bank'].'" onclick="return confirm(\'Anda yakin?\')"><i class="fa fa-trash-o"></i></a>
                            <a class="text-dark" href="cbank/update/'.$aRow['bank'].'"><i class="fa fa-pencil"></i></a>';
                            for ( $i=0 ; $i<$iColumnCount ; $i++ ) {
                                $row[] = $aRow[ $aColumns[$i] ];
                            }
                            $row = array($btn, $aRow['bank'], $this->fungsi->dateToIndo($aRow['lastupdate']), $aRow['user_id']);
                            $output['aaData'][] = $row;
                    }
	...
	public function delete($id_bank){
        $this->bank->delete_by_id_bank($id_bank);
    ...
    function save_update(){
        ...
        $data = array(
            'bank' => toAscii($this->input->post('bank')),
            'lastupdate' => $last,
            'user_id' => $nm,
        );
